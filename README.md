...
画面はスマホ対応済みです（640px以下では料金表を市場ごとのカードに組み替え、予約リンクを全幅ボタンにする）。サーバーは `0.0.0.0` で待ち受けるので、**同じWi-Fiにいれば、PCで起動したものをスマホのブラウザから開けます**。

```bash
# PC側で起動したまま、PCのLAN IPを確認する
ipconfig getifaddr en0        # macOS
hostname -I                   # Linux
# → スマホのブラウザで http://192.168.x.x:3000 を開く
```

`API_TOKEN` を設定している場合は、初回だけ `http://192.168.x.x:3000/?token=<値>` で開いてください。以降はその端末に保存されます。

注意点が2つあります。

- **料金取得用のChromeはPC（サーバー）側で動きます。** スマホはあくまで操作画面で、スマホのIPやブラウザは比較結果に影響しません。PC側のChromeを閉じると比較は失敗します。
- 外出先から使いたい場合は、VMにデプロイするか、トンネル（Cloudflare Tunnel、Tailscale 等）を使ってください。その際は `API_TOKEN` を必ず設定してください。
